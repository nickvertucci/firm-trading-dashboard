...
def firm_rvol_gainers_card():
    """Create a card displaying the top firm rVol gainers with position change indicators"""
    ...
    with ui.card().classes("w-64 p-4 shadow-md"):
        ...
        async def update_card():
            """Update the card with the latest firm rVol gainers and position changes"""
            ...
            with content:
                if not quotes:
                    ...
                    # Create a grid for table layout with an extra column for arrows
                    with ui.grid(columns=4).classes("w-full gap-1"):
                        ...
                        # Data rows (up to 10 from the API default size)
